...
async def get_site_statistics(session: AsyncSession) -> dict[str, int]:
    ...
    bookmarks_count_result = await session.execute(select(func.count()).select_from(Bookmark))
    bookmarks_total = bookmarks_count_result.scalar() or 0

    # Get total comments count
    comments_count_result = await session.execute(select(func.count()).select_from(Comment))
    comments_total = comments_count_result.scalar() or 0

    # Get total contents count
    contents_count_result = await session.execute(select(func.count()).select_from(Content))
    ...
    youtubers_count_result = await session.execute(select(func.count()).select_from(YouTuber))
